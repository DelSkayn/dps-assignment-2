...
func (chord *Chord) successor(on *FingerEntry) (*FingerEntry, error) {
	...
	client.Close()
	log.Debugf("Successor found: %v", result)
	return &result, nil
}
...
func (chord *Chord) notify(on *FingerEntry, of *FingerEntry) error {
	args := NotifyArgs{
		NodeID:   on.NodeID,
		Previous: of,
	}
	...
	err = client.Call("ChordNode.Notify", &args, result)
	if err != nil {
		return err
	}
	client.Close()
...
func (chord *Chord) initializeWithBootstrap(bootstrap *net.TCPAddr) {
	...
	for i := 0; i < nodes.Len(); i++ {
		...
		go func(i int) {
			for {
				successor, err := chord.FindSuccessor(finger, node.nodeID)
				if err != nil {
				...
				for i := 0; i < maxFingers; i++ {
					node.table.entries = append(node.table.entries, nil)
				}
				log.Debugf("successor %+v", successor)
				log.Debugf("address%+v", successor.Address)
				node.setSuccessor(successor, 0)
				node.lock.Unlock()
				// Start the stabilization process
				go node.Stabilize(chord)
				break
			}
		}(i)
	}
